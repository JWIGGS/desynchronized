...
if(clipCurrent>0 or clipSize = 0){
	...
	if(weapon_get_data(weaponDataShotAmount,weapon) = 1){
		...
		for(var i = -weapon_get_data(weaponDataDeviation,weapon); i<weapon_get_data(weaponDataDeviation,weapon); i +=weapon_get_data(weaponDataDeviation,weapon)*2/weapon_get_data(weaponDataShotAmount,weapon)){
	...
	//still has bullets
	audio_play_sound_player(asset_get_index("snd_"+weapon),playerControl);

	global.playerAction[value_to_1d(playerNumber,teamNumber,global.roundTotal),global.timeCurrent] = "shoot";
